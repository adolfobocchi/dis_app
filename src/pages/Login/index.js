...
import { loginRequest } from '../../store/modules/Usuario/actions';
...
const LoginPage = ({fetchLogin, loading, error}) => {
  ...
  return (
    <LoginContainer>
      <LoginForm method='POST' onSubmit={(event) => handleSubmit(onSubmit)(event)}>
        <LeftPanel>
          <AnimatedIcon src={require('../../assets/images/loginicon.jpg')}viewBox="0 0 24 24" />
        </LeftPanel>
        <RightPanel>
          <Title>Login</Title>
          <InputContainer>
...
const mapDispatchToProps = dispatch => {
  return {
    fetchLogin: (email,password, history) => dispatch(loginRequest(email,password,history))
  };
};
