...
function* searchEmpresas(action) {
  try {
    const {nomeFantasia, cnpj} = action.payload.query;
    const response = yield call(() => api.get(`/empresas/search?nomeFantasia=${nomeFantasia}&cnpj=${cnpj}`));
    const empresa = response.data;
    yield put({ type: SEARCH_EMPRESAS_SUCCESS, payload: empresa});
  } catch (error) {
    yield put({ type: SEARCH_EMPRESAS_FAILURE, payload: error.message });
  }
}

function* searchGrupos(action) {
  try {
    const {nome} = action.payload.query;
    console.log(nome)
    const response = yield call(() => api.get(`/grupos/search?nome=${nome}`));
    const empresa = response.data;
    yield put({ type: SEARCH_GRUPOS_SUCCESS, payload: empresa});
  } catch (error) {
    yield put({ type: SEARCH_GRUPOS_FAILURE, payload: error.message });
  }
}
